 ...
 * Expo's own push service (see docs/SCHEDULER_DESIGN.md's original plan vs. the later Azure
 * decision). Gets the RAW FCM device token (not Expo's wrapped one), since the server hands it
 * directly to Notification Hubs, which relays to FCM itself. Uses the device's own stable id as
 * the installation id, so re-registering (e.g. every sign-in) is an idempotent overwrite, not a
 * growing list of stale installations per device.
 */
